...
public class WikiTable implements Serializable {
     ...
     * @param header
     */
    public void assignColumnHeaders(List<List<WikiColumnHeader>> header) {
        columns = new WikiColumnHeader[header.size()][];
        int total_cols = 0;
        for (int i = 0; i < header.size(); i++) {
            List<WikiColumnHeader> cols = header.get(i);
            total_cols = cols.stream().mapToInt(col -> col.col_span).sum();

            //assign the matrix dimension only in the first column header, there we will know the complete length
            if (i == 0) {
                columns = new WikiColumnHeader[header.size()][total_cols];
            }
